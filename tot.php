...
function sendPasswordEmail($email, $random_password)
{
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'totoiscabs@gmail.com'; // SMTP username
        $mail->Password   = 'ecgi nuwe kuns rmrh';    // SMTP password (Use App Password for Gmail)
        $mail->SMTPSecure = 'tls';                 // Enable TLS encryption
        $mail->Port       = 587;                   // TCP port to connect to

        //Recipients
        $mail->setFrom('totoiscabs@gmail.com', 'Your Company Name');
        $mail->addAddress($email); // Add the recipient's email

        //Content
        $mail->isHTML(true);  // Set email format to HTML
        $mail->Subject = 'Your new password from Totoiscabs & Travel';
        $mail->Body    = "Your temporary password is: <h3>$random_password</h3>. Please use this to reset your password.";
